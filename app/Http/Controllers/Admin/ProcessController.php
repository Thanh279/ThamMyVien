...
class ProcessController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'image_titles.*' => 'nullable|string',
            'image_descriptions.*' => 'nullable|string',
            'delete_images.*' => 'nullable|exists:process_images,id',
            'existing_images.*' => 'nullable|exists:process_images,id',
        ]);
        ...
        if ($request->has('delete_images')) {
            foreach ($request->delete_images as $imageId) {
                $processImage = ProcessImage::find($imageId);
                if ($processImage) {
                    Storage::disk('public')->delete($processImage->image);
                    $processImage->delete();
                }
            }
        }
        ...
        if ($request->has('images')) {
            foreach ($request->images as $index => $imageFile) {
                if ($imageFile && $imageFile->isValid()) {
                    ...
                    if ($request->has('existing_images') && isset($request->existing_images[$index])) {
                        $existingImage = ProcessImage::find($request->existing_images[$index]);
                        if ($existingImage) {
                    ...
                    // Update title and description for existing images if no new image is uploaded
                    if ($request->has('existing_images') && isset($request->existing_images[$index])) {
                        $existingImage = ProcessImage::find($request->existing_images[$index]);
                        if ($existingImage) {
