                ...
                <select name="service_id" class="form-control">
                    @foreach ($services as $service)
                        <option value="{{ $service->id }}"
                            {{ old('service_id', $reason->service_id) == $service->id ? 'selected' : '' }}>
                            {{ $service->name }}</option>
                    @endforeach
                </select>
            </div>
                ...
                @for ($i = 0; $i < 4; $i++)
                    <div class="image-item mb-3 border p-3">
                        <div class="mb-2">
                            ...
                            @if (isset($reason->processImages[$i]))
                                <img src="{{ asset('storage/' . $reason->processImages[$i]->image) }}" width="120"
                                    alt="Current Image">
                                <input type="hidden" name="existing_images[{{ $i }}]"
                                    value="{{ $reason->processImages[$i]->id }}">
                                <div>
                                    <input type="checkbox" name="delete_images[]"
                                        value="{{ $reason->processImages[$i]->id }}"> Xóa ảnh này
                                </div>
                            @else
                                <p>Chưa có ảnh</p>
                            @endif
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" name="image_titles[{{ $i }}]" class="form-control"
                                value="{{ old('image_titles.' . $i, isset($reason->processImages[$i]) ? $reason->processImages[$i]->title : '') }}">
                        </div>
                        <div class="mb-2">
                            <label>Mô tả</label>
                            <textarea name="image_descriptions[{{ $i }}]" class="form-control">{{ old('image_descriptions.' . $i, isset($reason->processImages[$i]) ? $reason->processImages[$i]->description : '') }}</textarea>
                        </div>
                    </div>
                @endfor
            </div>
